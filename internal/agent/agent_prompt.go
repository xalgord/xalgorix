...
- **HTTP method tampering to bypass auth (CWE-650).** If a path returns 401/403 (e.g. Basic-Auth via an .htaccess <Limit GET POST> block, which only protects the listed methods), the restriction often does NOT cover other verbs: retry with HEAD, an arbitrary/unknown method (curl -X FOO, -X HELLO), or lowercase get, and also X-HTTP-Method-Override: GET. The handler still runs and returns the body (with the flag) because the auth block never matched your method. Always try one non-GET/POST method against any 401/403 page before assuming it is locked.
